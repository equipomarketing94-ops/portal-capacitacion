...
import { collection, getDocs, doc, updateDoc } from 'firebase/firestore'; // NUEVO: Importamos updateDoc
...
export default function AdminDashboard() {
  ...
  // --- NUEVA FUNCIÓN: EL INTERRUPTOR DEL EXAMEN ---
  const toggleExamen = async (colaboradorId, estadoActual) => {
    try {
      // 1. Apuntamos al documento exacto de ese colaborador en Firebase
      const colaboradorRef = doc(db, "colaboradores", colaboradorId);
      
      // 2. Le decimos a Firebase: "Cambia el estado del examen a lo contrario de lo que está ahora"
      const nuevoEstado = !estadoActual;
      await updateDoc(colaboradorRef, {
        examenHabilitado: nuevoEstado
      });

      // 3. Actualizamos la "memoria" (useState) para que el botón cambie de color sin recargar la página
      ...
      alert("Hubo un error al intentar cambiar el estado del examen.");
    }
  };
  // ------------------------------------------------
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="p-6 md:p-8 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-50/50">
            <div>
              ...
              <p className="text-sm text-slate-500">Administra los accesos a las evaluaciones.</p>
            </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100 text-xs font-bold text-slate-400 uppercase tracking-widest">
                  ...
                  <th className="p-6 hidden md:table-cell">Estado</th>
                ...
                {filtrados.map((colab) => (
                  <tr key={colab.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="p-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-400 mt-1">{colab.documento}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="p-6 text-center">
                      ...
                      >
                        {colab.examenHabilitado ? <Unlock size={16} /> : <Lock size={16} />}
