...
import { Timer, Send, AlertCircle, CheckCircle2, Loader2 } from 'lucide-react';
import { db } from '../firebase';
import { doc, updateDoc, increment } from 'firebase/firestore';

// 1. EL CONTENIDO DEL EXAMEN (Lo separamos para protegerlo)
function ContenidoDelExamen() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const moduloId = searchParams.get('modulo') || '1';

  const TIEMPO_INICIAL = 1800; // 30 minutos
    ...
    try {
      ...
      if (documentoId) {
        ...
        await updateDoc(colaboradorRef, {
          progresoTotal: increment(15),
          examenHabilitado: false // Vuelve a bloquear el candado
        });
      }
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 text-center">
        <div className="bg-white p-10 rounded-[3rem] shadow-xl max-w-md border border-slate-100">
          ...
          <button onClick={() => router.push('/progreso')} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase tracking-widest hover:bg-orange-600 transition-all">
            Volver al Portal
          </button>
        </div>
      </div>
      ...
      <nav className="bg-slate-900 text-white p-4 sticky top-0 z-20 shadow-lg border-b border-orange-500">
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xl font-black tabular-nums">{formatearTiempo(segundosRestantes)}</span>
          </div>
          <h1 className="text-xs font-black uppercase tracking-[0.2em] hidden sm:block">Evaluación Módulo {moduloId}</h1>
          <button onClick={finalizarExamen} disabled={enviando} className="bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2">
            {enviando ? <Loader2 className="animate-spin" size={14} /> : <Send size={14} />} Finalizar
          </button>
        </div>
      </nav>
      ...
      <div className="max-w-3xl mx-auto p-6 mt-8">
        <div className="bg-orange-50 border border-orange-100 p-6 rounded-3xl flex gap-4 mb-8">
          ...
          <p className="text-sm text-orange-800 font-medium">
            <strong>Atención:</strong> Tienes 30 minutos para completar esta prueba. Si el tiempo llega a cero, el examen se enviará automáticamente.
          </p>
        </div>
...
// 2. LA PÁGINA PRINCIPAL ENVUELTA EN LA CAJA PROTECTORA
export default function PaginaExamen() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white">
        <Loader2 className="animate-spin text-orange-500 mb-4" size={48} />
        <p className="text-[10px] font-black uppercase tracking-widest">Preparando Examen...</p>
      </div>
    }>
      <ContenidoDelExamen />
    </Suspense>
